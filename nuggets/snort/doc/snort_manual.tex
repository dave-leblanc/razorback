...
\texttt{<path>} specifies the directory for snort to creat the socket.
...
\begin{itemize}
   ...
   \item Start shared memory master(writer) with \texttt{-G} 1 option. Note: only one master
      should be enabled.
   \item Start shared memory clients (readers) with \texttt{-G}  2 or other IDs. Note: for 
