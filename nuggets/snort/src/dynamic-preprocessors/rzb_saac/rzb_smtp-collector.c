...
void
smtpdumper_freedata (smtpcapture * sessiondata)
{
...
void
smtpdumper_calldetection (void *dataptr)
{
...
//        printf
//            ("SMTPDUMP Calling sendData() with the following data (%d bytes):\n\n",
//             ((smtpcapture *) (dataptr))->storedsize);
#ifdef DISPLAY_DEMO_OUTPUT
//        prettyprint (smtpcapturedata->clientdata,
//                     smtpcapturedata->storedsize);
//        printf ("\n\n");
#endif


        // Fill in the required fields
        if ((l_pBlock = BlockPool_CreateItem (g_pContext)) == NULL)
        {
            DEBUG_WRAP(DebugMessage(DEBUG_RZB, "Failed to allocate new block"));
            return;
        }

        BlockPool_AddData (l_pBlock, smtpcapturedata->clientdata,
                           smtpcapturedata->storedsize,
                           BLOCK_POOL_DATA_FLAG_MALLOCD);
        BlockPool_SetItemDataType (l_pBlock, DATA_TYPE_SMTP_CAPTURE);
        BlockPool_FinalizeItem (l_pBlock);
        l_pBlock->submittedCallback = NULL;


        Submission_Submit (l_pBlock, 0, &l_iSf_Flags, &l_iEnt_Flags);
...
/* detection functions */
int
smtpdumpereval (SFSnortPacket * sp)
{
...
//   Packet *sp = (Packet *) p;
    ...
    sessiondata =
        _dpd.streamAPI->get_application_data (sp->stream_session_ptr,
                                              SAAC_SMTP);
    ...
    if (sessiondata)
    {
        ...
        {
            //printf
            //    ("SMTPDUMP Already have SMTPCAP_MAXSIZE(%d) bytes of data\n",
            //     SMTPCAP_MAXSIZE);
            return RULE_NOMATCH;
        }
    }
    else
    {
        ...
        _dpd.streamAPI->set_application_data (sp->stream_session_ptr,
                                              SAAC_SMTP, sessiondata,
                                              &smtpdumper_calldetection);
...
//    DEBUGOUT ((D_PACKET | D_WARN),
//              prettyprint (sp->payload, sp->payload_size));

    //printf ("SMTPDUMP incoming_data_size = %d\n", incoming_data_size);

    // Check if we have enough room for the incoming data
    if (incoming_data_size >
        (sessiondata->totalsize - sessiondata->storedsize))
    {
        ...
        tmpdataptr =
            realloc (sessiondata->clientdata, sessiondata->totalsize * 2);
