        ...
        const u_char *end, HEADER_PTR *header_ptr, sfip_t **true_ip)
{
    ...
    {
        p++;
        sf_unfold_header(p, end-start, unfold_buf, sizeof(unfold_buf), &unfold_size, 0 , &num_spaces);
        ...
        const u_char *end, HEADER_PTR *header_ptr, HEADER_FIELD_PTR *header_field_ptr)
{
      ...
      (header_field_ptr->content_len->cont_len_end) )
    {
        char *pcEnd;
        uint64_t len;
        header_field_ptr->content_len->len = len =
            SnortStrtol((char *)header_field_ptr->content_len->cont_len_start, &pcEnd, 10);
        ...
        HttpSessionData *hsd, int stream_ins, int *host_name_header)
{
    if (((p - offset) == 0) && ((*p == 'C') || (*p == 'c')))
    {
        ...
        else if ( IsHeaderFieldName(p, end, HEADER_NAME__CONTENT_LENGTH, HEADER_LENGTH__CONTENT_LENGTH) )
        {
            p = extract_http_content_length(Session, ServerConf, p, start,
                                            end, header_ptr, header_field_ptr );
        }
    }
    else if (((p - offset) == 0) && ((*p == 'x') || (*p == 'X') || (*p == 't') || (*p == 'T')))
    {
                                    ...
                                    HEADER_LENGTH__TRANSFER_ENCODING) && hsd)
        {
            ...
            p = extract_http_transfer_encoding(Session, hsd, p, start, end, header_ptr, HI_SI_CLIENT_MODE);
        }
    }
