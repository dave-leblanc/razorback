...
static void
add_topools (threadpool_t * t)
{
    struct threadpool_list *new = malloc (sizeof (*new));
    if (!new)
    {
        rzb_log (LOG_ERR, "Unable to add threadpool to list");
        return;
    }
    new->pool = t;
...
static void
remove_frompools (threadpool_t * t)
{
...
static void
print_queue (int f, work_queue_t * queue, struct timeval *tv_now)
{
...
int
thrmgr_printstats (int f, char term)
{
    struct threadpool_list *l;
    unsigned cnt, pool_cnt = 0;
    size_t pool_used = 0, pool_total = 0, error_flag = 0;
    float mem_heap = 0, mem_mmap = 0, mem_used = 0, mem_free =
        0, mem_releasable = 0;
    ...
    for (l = pools; l && !error_flag; l = l->nxt)
    {
        ...
        mdprintf (f, "QUEUE: %u items",
                  pool->single_queue->item_count +
                  pool->bulk_queue->item_count);
        ...
        for (task = pool->tasks; task; task = task->nxt)
        {
            ...
            mdprintf (f, "\t%s %f %s\n",
                      ...
                      delta / 1e6, task->filename ? task->filename : "");
        }
        mdprintf (f, "\n");
    }
    free (seen);
    ...
    else
    {
        if (has_libc_memstats)
            mdprintf (f,
                      ...
                      mem_heap, mem_mmap, mem_used, mem_free, mem_releasable,
                      pool_cnt, pool_used / (1024 * 1024.0),
                      pool_total / (1024 * 1024.0));
        else
            mdprintf (f,
...
thrmgr_destroy (threadpool_t * threadpool)
{
    ...
    {
        rzb_log (LOG_ERR, "Mutex lock failed");
        exit (-1);
    }
    if (threadpool->state != POOL_VALID)
    {
        ...
        {
            rzb_log (LOG_ERR, "Mutex unlock failed");
            exit (-1);
        }
        return;
    }
    ...
    while (threadpool->thr_alive > 0)
    {
        if (pthread_cond_wait
            (&threadpool->pool_cond, &threadpool->pool_mutex) != 0)
        {
            pthread_mutex_unlock (&threadpool->pool_mutex);
            return;
        }
...
threadpool_t *
thrmgr_new (int max_threads, int idle_timeout, int max_queue,
            void (*handler) (void *))
{
    ...
    if (pthread_attr_setdetachstate
        (&(threadpool->pool_attr), PTHREAD_CREATE_DETACHED) != 0)
    {
        pthread_cond_destroy (&(threadpool->queueable_single_cond));
        pthread_cond_destroy (&(threadpool->queueable_bulk_cond));
        pthread_attr_destroy (&(threadpool->pool_attr));
...
#endif
            stacksize = 1048576;
    rzb_log (LOG_ERR, "Set stacksize to %lu", (unsigned long int) stacksize);
...
void
thrmgr_setactiveengine (const struct cl_engine *engine)
{
    struct task_desc *desc;
    pthread_once (&stats_tls_key_once, stats_tls_key_alloc);
    desc = pthread_getspecific (stats_tls_key);
...
/* thread pool mutex must be held on entry */
static void
stats_destroy (threadpool_t * pool)
{
...
static inline int
thrmgr_contended (threadpool_t * pool, int bulk)
{
...
thrmgr_pop (threadpool_t * pool)
{
    ...
    {
        rzb_log (LOG_DEBUG, "THRMGR: queue (single) crossed low threshold -> signaling");
        pthread_cond_signal (&pool->queueable_single_cond);
    }

    if (!thrmgr_contended (pool, 1))
    {
        rzb_log (LOG_DEBUG, "THRMGR: queue (bulk) crossed low threshold -> signaling");
...
thrmgr_worker (void *arg)
{
    ...
    for (;;)
    {
        if (pthread_mutex_lock (&(threadpool->pool_mutex)) != 0)
        {
            rzb_log (LOG_ERR, "Fatal: mutex lock failed");
            exit (-2);
        }
        if (!stats_inited)
        {
        ...
        if (pthread_mutex_unlock (&(threadpool->pool_mutex)) != 0)
        {
            rzb_log (LOG_ERR, "Fatal: mutex unlock failed");
            exit (-2);
        }
        if (job_data)
        {
...
static int
thrmgr_dispatch_internal (threadpool_t * threadpool, void *user_data,
                          int bulk)
{
    int ret = true;
    pthread_t thr_id;
    ...
    do
    {
        ...
        {
            rzb_log (LOG_DEBUG, "THRMGR: contended, sleeping");
            pthread_cond_wait (queueable_cond, &threadpool->pool_mutex);
            rzb_log (LOG_DEBUG, "THRMGR: contended, woken");
        }
        ...
        items =
            threadpool->single_queue->item_count +
            threadpool->bulk_queue->item_count;
...
int
thrmgr_group_dispatch (threadpool_t * threadpool, jobgroup_t * group,
                       void *user_data, int bulk)
{
    int ret;
    if (group)
    {
        ...
        group->jobs++;
        rzb_log (LOG_DEBUG, "THRMGR: active jobs for %p: %d", group, group->jobs);
        pthread_mutex_unlock (&group->mutex);
    }
    if (!(ret = thrmgr_dispatch_internal (threadpool, user_data, bulk))
        && group)
    {
        pthread_mutex_lock (&group->mutex);
        group->jobs--;
 ...
 */
int
thrmgr_group_finished (jobgroup_t * group, enum thrmgr_exit exitc)
{
    int ret = 0;
    if (!group)
    {
    ...
    pthread_mutex_lock (&group->mutex);
    rzb_log (LOG_DEBUG, "THRMGR: group_finished: %p, %d", group, group->jobs);
    group->exit_total++;
    switch (exitc)
    {
    case EXIT_OK:
    ...
    if (ret)
    {
        rzb_log (LOG_DEBUG, "THRMGR: group_finished: freeing %p", group);
...
void
thrmgr_group_waitforall (jobgroup_t * group, unsigned *ok, unsigned *error,
                         unsigned *total)
{
    ...
    *total = group->exit_total;
    if (!--group->jobs)
        ...
        rzb_log (LOG_ERR, "$THRMGR: active jobs for %p: %d", group, group->jobs);
    pthread_mutex_unlock (&group->mutex);
    if (needfree)
    {
        rzb_log (LOG_ERR, "$THRMGR: group finished freeing %p", group);
        free (group);
    }
}

jobgroup_t *
thrmgr_group_new (void)
{
    jobgroup_t *group;
    ...
    if (!group)
        return NULL;
    group->jobs = 1;
    group->exit_ok = group->exit_error = group->exit_total =
        group->force_exit = 0;
    if (pthread_mutex_init (&group->mutex, NULL))
    {
        rzb_log (LOG_ERR, "^Failed to initialize group mutex");
        free (group);
        return NULL;
    }
    if (pthread_cond_init (&group->only, NULL))
    {
        rzb_log (LOG_ERR, "^Failed to initialize group cond");
        pthread_mutex_destroy (&group->mutex);
        free (group);
        return NULL;
    }
    rzb_log (LOG_ERR, "$THRMGR: new group: %p", group);
    return group;
}

int
thrmgr_group_need_terminate (jobgroup_t * group)
{
    int ret;
    if (group)
    {
...
void
thrmgr_group_terminate (jobgroup_t * group)
{
    if (group)
    {
